...
// Recall Trail
// Approach: started from a 2D memoized recursion rec(i, left) (min coins
// to make `left` using only coins from index i onward, i staying fixed on
// take = reuse, advancing on skip). Converged that this works but wastes
// memory/time (~10^8 states) and, if implemented as actual recursion,
// would blow the stack at x up to 10^6.
// Decision points:
...
//   at every state regardless of history. Collapsed the 2D rec(i,left)
//   into 1D dp[left] by replacing "skip coin i / take coin i" branches
//   with one loop over all coin values per state:
...
// - Bottom-up loop (not top-down recursion) to avoid stack depth ~x = 10^6.
// See also Dice Combinations - same "1D bottom-up, sum/min over prior
// states" family, but that one sums over a fixed window (dice faces 1-6,
// order matters) while this one takes a min over an arbitrary coin set
// (order doesn't matter, unbounded reuse).
